...
#parser already extracts hour from date, so all we need is defining key and value for reduceByKey

if __name__ == "__main__":
    conf = SparkConf().setAppName("vinastya_3_spark_app").setMaster(sys.argv[1]).set("spark.ui.port", "4090")
    sc = SparkContext(conf=conf)
    lines = sc.textFile("%s" % sys.argv[2])
    objects = lines.map(parseLine)

    #use "bytes" as value instead of 1s
    cnt_bytes_by_ip = objects.map(lambda line_tuple: (line_tuple[-1], 1)) \
